...
- `[ ]` C5: Advanced composites: masks, resolution-aware composites, lookup tables, fill, auxiliary data, cloud products, lightning overlays, SAR, and config loading.
  - `[x]` C5-m9d1: Add `DayNightCompositor` foundation (Satpy `fill.py` `day_night="day_night"`) blending corrected (day) and uncorrected (night) band-major RGB composites with per-pixel solar-zenith weights, plus `daynight_blend_weights` in `rusty_sat_modifiers` (cos-zenith weights, Satpy `DayNightCompositor._get_coszen_blending_weights` parity) and the JMA `true_color_reproduction` integration path (lim_low 73°/lim_high 85°).
...
| `EwaResampler`: dependency-free lon/lat swath-to-geographic-area EWA-style weighted accumulation for 2D f64 grids with rayon-parallel lock-free atomic accumulation (identical memory footprint), configurable radius/weight/fill/masked-missing policy, source-mask skipping, destination x/y coordinates, metadata preservation, owned resampling, and typed pipeline preparation through `SourceGeometry::Swath` | Full Pyresample Fornav/LLS2 parity, scan-aware `rows_per_scan`, maximum-weight mode, chunked execution, multi-band sampling, geocentric/cross-projection distances, and production EWA performance |
| `NativeResampler`: Satpy-style native repeat for integer upscaling, nanmean aggregation for integer downscaling, equal-shape pass-through, mixed-axis rejection, mask propagation, destination x/y coordinates, metadata preservation, higher-dimensional arrays by resampling named `y,x` axes with stack-allocated index arithmetic (no per-pixel heap allocation), runtime-typed identity/repeat that preserves `f32`/`f64`/`u8`/`u16`/`i16`, and aggregate means promoted to `f64` | Lazy/chunked native execution and full Satpy area-choice integration |
...
| Execute `SelfSharpenedRgb` (high-res band sharpening, Satpy `resolution.py`) and `DayNightCompositor` (corrected/uncorrected band-major RGB blend with per-pixel SZA weights, Satpy `fill.py`; weights supplied externally) with mask propagation and owned variants | DayNight alpha/mask-weight parity (`include_alpha`, `_mask_weights_with_data`), single-side day/night-only modes, and Scene dependency execution |
...
| Can | Cannot |
|-----|--------|
| `RayleighCorrector`: full Rayleigh scattering correction with pyspectral LUT loading, trilinear interpolation (binary-search interval lookup, bit-identical to rustyspectral), pyspectral-parity red-band cloud relaxation (`RedBandSource`: `None`/`Dataset`/`SunZenithCorrectedVis`), LUT-boundary angle clipping (hoisted per correction), correction clip to [0,100], high-zenith reduction, and 64-row strip-based parallel execution consuming the dataset; the combined sun-zenith+Rayleigh path applies the sunz amplification unconditionally and the Rayleigh subtraction only for wavelengths inside the LUT range (Satpy `[sunz_corrected]`-only semantics for e.g. AHI B04) | Full GSICS inter-calibration parity beyond user-supplied RAD coefficients; real multi-sensor production fixture tests |
| `RayleighCorrector` batched corrections: `apply_corrections_with_sun_zenith_batch` corrects several bands on one grid in a single pass (angles computed once per pixel; per-band in-place red prerequisites; bit-identical outputs) and `apply_correction_with_sun_zenith_and_weights` fuses the `DayNightCompositor` weight emission into the correction | Batch red prerequisites from external datasets; angle caching across different grids |
| `SunZenithCorrector`: normalizes TOA reflectance to overhead-sun equivalent via Satpy `sunzen_corr_cos` (1/cos with 88°→max_sza ANGLE-domain gradient falloff, continuous at 88°) in 64-row strips with Earth-disk boundary detection and consuming dataset API; `daynight_blend_weights` computes Satpy `DayNightCompositor` cos-zenith blend weights (lim_low/lim_high) in strips for corrected/uncorrected composites | Solar path-length / airmass variants beyond simple cos(sza); production full-disk fixture parity tests |
| `GeosProjection::inverse`: exact ellipsoidal ray–ellipsoid geos inverse (PROJ `geos` / pyresample `get_lonlats` parity, verified to 1e-9 against limb reference values; replaces the flat-plane approximation that was off by ~57° at the disk limb), with `inverse_rad`/`inverse_batch`/`lonlat_grid` variants | Projected cross-CRS transforms outside geos; stacked/dynamic areas |
| Compute solar/satellite azimuth/zenith angles via `AngleSet` (full-grid) or on-the-fly per-pixel in strips, both through the exact geos inverse + pyorbital solar/satellite formulas; `AngleParams` extracts projection geometry from dataset area attrs | Angle computation for non-geostationary sensors; ECI/ECEF satellite position beyond geos |
...
- `KdPointIndex2D` construction now uses `select_nth_unstable` (O(n log n)) but still stores ~48 bytes-per-node `Vec<KdNode>`. For a production VIIRS swath (~20.5M valid points) this means ~984 MB tree memory. Consider `f32` coordinate storage or compressed node layouts when the first real swath reader lands.
...
- `FloatTiffWriter` still materializes a full encoded byte buffer before writing pixels. The dtype-aware encode (e3182c1) removed the `values_as_f64()` intermediate and the strip-mode `bytes.clone()`, and tile/deflate paths now drop the contiguous source as soon as blocks are produced (DEFLATE tile compression is rayon-parallel). Remaining work: row/strip/tile streaming to avoid the output byte buffer entirely for large AHI products.
- `ValidityMask::extend` uses per-bit iteration with `is_masked()` calls for each appended bit. For a B03 full-disk assembly (484M px across 10 segments) this is ~7.3B CPU instructions for mask accumulation alone — but it only accounts for an estimated ~5% of load+cal time (~0.8s out of ~16s, now lower with parallel segments). Future work: byte-level copy with shift/OR for boundary bytes (~60× fewer instructions). Not urgent; the critical memory benefit (packed instead of Vec<bool>) is already achieved.
- AHI HSD segment loading/calibration is rayon-parallel in chunks of `CONCURRENT_SEGMENT_LOADS = 4` (`ahi_hsd.rs`). Tradeoff: peak assembly memory is `assembled buffer + 4 concurrent segment buffers` (e.g. ~680 MB f32 B03/B04 full-disk paths) instead of `assembled + 1 segment`. A fully streaming single-segment mode remains possible for tight-memory hosts.
- `SceneLoader::load_batch` (readers macro) loads all requested datasets from a reader concurrently with rayon. Multi-band loads therefore hold several full-disk buffers at once; readers with large per-dataset footprints can override `load_batch` to bound concurrency.
- The exact geos inverse (`geos.rs::inverse_rad_inner`) adds a ray–ellipsoid quadratic solve + geodetic-latitude Newton iteration per pixel to the sun-zenith/Rayleigh strip loops (~1.3–1.6× the per-pixel cost of the old flat-plane approximation, which was unusably wrong at the limb). The quadratic coefficients `A` (separable in x/y) and `B`/`C` (constants) already avoid redundant work; a fused spherical-start inverse could shave the remaining trig when modifier throughput on full-disk 0.5 km imagery needs it.
- The true-color red band test (`true_color_reproduction.rs`) promotes the 0.5 km B03 raw band to f64 for the f64-only area nearest resampler (`to_f64_dataset`), allocating a temporary ~3.9 GB f64 copy that is dropped right after the 0.5→1 km resample. A runtime-typed area nearest resampler (P0.1.1d pending work) would remove the promotion.
- The JMA `true_color_reproduction` integration path blends the two u8 renders (`blend_daynight_u8` in `true_color_reproduction.rs`) instead of blending the CIRA-stretched f32 composites like Satpy's `DayNightCompositor`. The extra quantization costs at most 1 LSB per channel and keeps three u8 buffers instead of three 5.8 GB band-major f32 composites; the f32 `DayNightCompositor` in `rusty_sat_composites` is the faithful path for callers with the memory budget.
- EWA accumulation uses lock-free atomic f64 CAS adds (`AtomicU64` bit-pattern `fetch_update`). Parallel summation order is non-deterministic, so last-bit float differences between runs are possible; contention grows when many source points map to the same target pixel (large radius relative to pixel size). Memory footprint is identical to the sequential version.
- `nearest_rows` / bilinear row-parallel resampling is deterministic (rows are independent and concatenated in order); the lazy nearest path (`SourceChunkCache` FIFO) is intentionally still sequential because its chunk cache requires `&mut` state.
- `FloatImage` enhancement ops (stretch/gamma/invert/cira/finalize) are rayon-parallel per pixel; min/max reductions are deterministic (associative/commutative), per-pixel transforms are bit-identical to the sequential loops.
