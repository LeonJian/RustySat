...
- `[~]` S2: KD-tree nearest resampling: tree creation, neighbour info, sampled output, radius calculation, Gaussian weights, great-circle distances, chunked parallel processing, `BaseResampler`, and full swath-to-grid support.
  ...
  - `[ ]` S2-next: Add geocentric/great-circle KD distances, chunked parallel processing, multi-neighbour KD-tree acceleration, uncertainty outputs, and neighbour-info generation for swaths.
...
- `[~]` S6: Native resampler: repeat, aggregate, and native-resolution pipelines.
  ...
  - `[ ]` S6-next: Add generic dtype native resampling, chunked/lazy execution, and full Satpy native-resampler integration with Scene area-choice helpers.
...
| `NativeResampler`: f64 Satpy-style native repeat for integer upscaling, nanmean aggregation for integer downscaling, equal-shape pass-through, mixed-axis rejection, mask propagation, destination x/y coordinates, metadata preservation, and higher-dimensional arrays by resampling named `y,x` axes while preserving non-spatial dimensions | Generic dtype native resampling, lazy/chunked native execution, and full Satpy area-choice integration |
...
- `dataset_metadata_pairs` and `dataset_attr_pairs` in nearest resampling clone every metadata key/value and attrs key/value into intermediate `Vec`s before re-inserting them into the resampled dataset. The borrowed `resample` path still uses these; the owned `resample_owned` path iterates the source maps directly. Unify both paths to clone individual entries inline.
...
- `NeighbourInfo::source_index_for_reduced_index()` scans `valid_input_index` linearly per call. When `valid_input_index` includes masked-out pixels, repeated calls during sampling will pay O(source_size) per output pixel. Pre-build an index lookup table during construction.
