...
- `[ ]` M7-AHI-production: Complete production-ready AHI HSD and AHI NetCDF workflows before broad reader expansion.
  ...
  - `[~]` M7-AHI-production-e: Complete AHI NetCDF metadata/data/geometry loading. Roadmap: R1-AHI-NC/R0.2.
    ...
    - `[ ]` M7-AHI-production-e2: Add AHI L2 NetCDF variable data loading with Satpy-style Rows/Columns -> y/x handling. Roadmap: R1-AHI-NC-data/R0.2.
    - `[ ]` M7-AHI-production-e3: Attach AHI L2 NetCDF area metadata and x/y coordinates to loaded datasets for resampling. Roadmap: R1-AHI-NC-geometry/S7.
...
| AHI L2 NetCDF fixture reader/handler can validate Satpy `ahi_l2_nc` full-disk global attrs, capture sensor/platform/start/end metadata, expose YAML-mapped dataset IDs by AHI L2 file type, and produce the hardcoded Himawari full-disk area metadata used by Satpy | AHI L2 NetCDF variable data loading, native NetCDF backend IO, Rows/Columns -> y/x array conversion, mask/scale handling, coordinate attachment, and real sample parity tests |
