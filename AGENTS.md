...
| `SunZenithCorrector`: normalizes TOA reflectance to overhead-sun equivalent via Satpy `sunzen_corr_cos` (1/cos with 88°→max_sza gradient falloff) in 64-row strips with Earth-disk boundary detection and consuming dataset API | Solar path-length / airmass variants beyond simple cos(sza); production full-disk fixture parity tests |
